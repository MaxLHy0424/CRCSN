...
使用 GBK (ANSI) 编码打开软件目录下的`dat.cfg`, 根据说明修改配置.
...
如将此项配置为`NULL`, 则不会显示额外文本, 否则将会显示所配置的文本. 文本将会显示在导航栏和选项之间.
...
设置 "工具大全" 内 "自定义命令" 执行的命令.

如果将此项配置为`NULL`, 将视为配置 (即默认配置), 否则将会执行所配置的命令.
...
## 5 软件中显示配置文件中的字符时, 中文字符变成了乱码.

请检查配置文件的文字编码, 确保为 GBK (ANSI) 编码. 可以使用记事本打开配置文件, 选择另存为, 选择 GBK (ANSI) 编码保存; 或使用其他支持 GBK (ANSI) 编码的文本编辑器.
