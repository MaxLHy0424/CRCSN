...
## 5 软件目录下存在`dat.cfg`, 但软件提示无法读取.

请检查`dat.cfg`的属性 (右键`dat.cfg` > 属性 > 安全), 点击 "编辑", 依次点击上方列出的所有用户, 将下方的 "允许" 一栏全部勾选, "拒绝" 一栏全部取消勾选 (每栏的最后一个选项 "特殊权限" 可能无法 (取消) 勾选, 但并不影响), 再加载配置试试.
