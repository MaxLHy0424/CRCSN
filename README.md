...
建议先关闭操作系统上已安装的所有防病毒软件, 否则 CRCSN 的操作可能会被阻止.

## 破解

以管理员权限运行软件, 依次输入`1 1 Y`, 开始破解. 等到软件提示返回主时即代表完成破解.

如果发现控制软件的进程仍在运行, 请在返回主菜单后, 一次输入`1 2 0 Y`, 开始循环破解.
...
打开软件目录下的`config.ini` (推荐使用 Windows Notepad 或 Visual Studio Code), 根据说明修改配置 
