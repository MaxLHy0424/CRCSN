...
{
    console_ui ui;
    ui.set_console( WINDOW_TITLE, CODE_PAGE, WINDOW_WIDTH, WINDOW_HEIGHT, true, false, true, 255 ).lock( true, true );
    std::print( "-> 检测运行权限.\n" );
    ...
    ui.add_back( "                    [ 主  页 ]\n\n" )
      ...
      .add_back( "\n[破解]\n" )
      .add_back( std::format( " > {} ", core::custom_rules.showed_name ), core::rule_op{ 'c', core::custom_rules } );
    for ( const auto &rule : core::builtin_rules ) {
        ui.add_back( std::format( " > {} ", rule.showed_name ), core::rule_op{ 'c', rule } );
    }
    ui.add_back( "\n[恢复]\n" )
      .add_back( std::format( " > {} ", core::custom_rules.showed_name ), core::rule_op{ 'r', core::custom_rules } );
    for ( const auto &rule : core::builtin_rules ) {
