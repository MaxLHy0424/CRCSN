...
unsigned short Mode1(unsigned short SleepTime=0){
...
unsigned short Mode2(unsigned short SleepTime=0){
...
int main(){
    ...
    unsigned short SleepTime=0,InputWrongNumber=0;
    if(FeatureCode[0]=='1'||FeatureCode[0]=='2'){
        printf("请输入 \"休眠\" 的时间 (\"休眠\" 在每次执行命令后暂停一定时间, 单位 秒, 输入 0 禁用, 最大 30): ");
        scanf("%hu",&SleepTime);
        while(SleepTime>30){
